...
# Read the contact page for the shell
...
# Replace <main>...</main> entirely
template = re.sub(r'<main>.*?</main>', new_main_content, template, flags=re.DOTALL)

# Replace <title>
template = re.sub(r'<title>.*?</title>', '<title>Interactive Solutions & Calculators - Bongshai Housing</title>', template)

# Update nav active state (contact.html has contact as active sometimes, or solutions as inactive since we copied it)
template = template.replace('<a class="mobile-nav-link" href="solutions.html">Solutions</a>', '<a class="mobile-nav-link active" href="solutions.html">Solutions</a>')
template = template.replace('<a class="nav-link" href="solutions.html">Solutions</a>', '<a class="nav-link active" href="solutions.html">Solutions</a>')


with open(r"e:\web\Bongshaihousing\solutions.html", "w", encoding="utf-8") as f:
    f.write(template)

print("Successfully injected calculators into solutions.html")
