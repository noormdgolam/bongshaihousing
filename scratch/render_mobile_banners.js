...
const template = fs.readFileSync(path.resolve(__dirname, 'banner-template-mobile.html'), 'utf8');
...
(async () => {
  ...
  for (const slide of slides) {
    ...
    const tmpFile = path.resolve(__dirname, `_tmp_mob_${slide.id}.html`);
    fs.writeFileSync(tmpFile, html);

    const page = await browser.newPage({ viewport: { width: 1080, height: 2338 } });
    await page.goto('file://' + tmpFile);
    await page.waitForTimeout(300);
    await page.screenshot({ path: path.resolve(__dirname, 'banner-output', `${slide.id}-mobile-banner.png`) });
    await page.close();
