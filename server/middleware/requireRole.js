...
module.exports = function requireRole(...allowedRoles) {
  return function (req, res, next) {
    // No fallback default here on purpose - an unset/unrecognized role
    // must fail closed (denied) rather than silently getting editor-level
    // access. 'admin' isn't special-cased either; every call site already
    // lists it explicitly in allowedRoles, so a hardcoded bypass would
    // only add a second, easier-to-forget place that grants access.
